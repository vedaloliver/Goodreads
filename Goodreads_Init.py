...
def scrape():
    ...
    for i in page:
        url = requests.get('https://www.goodreads.com/review/list/' + str(user_id) + '?order=d&page=' +
                           str(i)+'&per_page=30&shelf=read&sort=date_added&utf8=%E2%9C%93&view=table')
        soup = BeautifulSoup(url.text, 'lxml')
        for book in soup.find_all('tr', class_='bookalike review'):
            name = book.find(
                'td', class_='field title').div.a.text.lstrip().rstrip().strip('\n')
            author = book.find(
                'td', class_='field author').div.a.text.lstrip().rstrip().strip('\n')
            page_no = book.find('td', class_='field num_pages').div.nobr.text.lstrip(
            ).rstrip().replace('pp', '').replace(' ', '').replace(',', '').strip('\n')
            date_added = book.find(
                'td', class_='field date_added').div.span.text.lstrip().rstrip().strip('\n')
            date_added_f = datetime.datetime.strptime(
                date_added, '%b %d, %Y').date()
            date_read = book.find('td', class_='field date_read').div.div.text.replace(
                '[edit]', '').lstrip().rstrip().strip('\n')
            date_published = re.findall(r".*([1-3][0-9]{3})", book.find(
                'td', class_='field date_pub').div.text.lstrip().rstrip().strip('\n'))
            date_published1 = ''.join(date_published)
            avg_rating = book.find('td', class_='field avg_rating').div.text.lstrip(
...
def quote_extract():
    ...
    def SQL_commit():
        ...
        cur.execute('''INSERT or REPLACE INTO Quote_link (quote_link)
            VALUES ( ? )''', (quotes, ))
        cur.execute(
            'SELECT id FROM Quote_link WHERE quote_link = ?', (quotes, ))
